...
const PixiComponent = ({ gameData, xpBubbles }) => {
  ...
  useEffect(() => {
    ...
    // Récupère les bulles d'xp de la base de données
    let xpBubblesList = [];
    ...
    /*apiXpBubble("GET").then((res) => {
      console.log(res);
      res.forEach((bubble) => {
        xpBubblesList.push(
          XpBubble(null, null, bubble.worldPosX, bubble.worldPosY)
        );
      });
    });*/
